...
class ImprovedNoise {
    ...
    private static let permutation: [Int] = [151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53,
    ...
    private static let p: [Int] = {
        var x = Array.init(repeating: 0, count: 512)
        for i in 0..<256 {
            x[i] = permutation[i]
            x[256 + i] = permutation[i]
        }
        return x
    }()
